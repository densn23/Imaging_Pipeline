...
EXCLUDE_BLOCKS = None  #"19-12-25 R4, 11-12-25 R3, 12-12-25 R1" #=J8  #"08-04-26 R9" #=(J10)    # e.g. "19-12-25 R4" or "Jamie8/19-12-25/R4, Jamie11/03-04-26/R18"
...
GROUP_COMPARISON_METRIC = "hilbert_amp_ratio"  # options: hilbert_amp_ratio, plv, plv_pooled, vm_early, vm_late, spta_amp, mpta_lat, mpta_jit, mpta_amp, peak_1_latency_ms_median, peak_1_jitter_ms, r_vm_velocity, r_hilbert_velocity, r_hilbert_vm
GROUP_A = {
    "label": "Group A",
    "mouse_name": "Jamie10",
    "frequency_hz": 135,
...
PLOT_SINGLE_PTA = True  # False/off, True/"normal", "derivative", or "both"
PLOT_MEAN_PTA = False
PLOT_LFP = False
PLOT_VELOCITY = False
...
PLOT_PLV_HISTOGRAMS = True  # 1=stim frequency, 2=2x stim, 3=3x stim; e.g. "1+2+3"
...
FULL_TRACE_XLIM_S = (-0.1, 0.5)  # None = auto
...
def plot_condition_average(blocks: list[BlockSpec], out_png: Path) -> None:
    panel_defs = []
    if PLOT_FULL_TRACE:
    ...
    for i, (ax, panel_name) in enumerate(zip(axes, panel_defs)):
        ...
        if panel_name == "full_trace":
            ...
            if FULL_TRACE_XLIM_S is None:
                set_tight_curve_xlim(ax, x, mean)
            else:
                ax.set_xlim(*FULL_TRACE_XLIM_S)
    ...
    add_panel_letters(fig, axes[:len(panel_defs)], ncols)

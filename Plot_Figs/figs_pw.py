...
IMAGING_SIDE = None  # "Left", "Right", or None
...
EXCLUDE_BLOCKS = "Jamie12/06-04-26/R4, Jamie8/09-12-25/R5, Jamie8/19-12-25/R4, Jamie8/31-12-25/R1, Jamie12/01-04-26/R3"      # e.g. "19-12-25 R4" or "Jamie8/19-12-25/R4, Jamie11/03-04-26/R18"
...
COMPARE_GROUPS = True
PLOT_GROUP_COMPARISON = True  # True = plot GROUP_A and GROUP_B as two rows instead of Selection
GROUP_COMPARISON_METRIC = "mpta_amp, mpta_jit, mpta_lat"  # options: hilbert_amp_ratio, plv, plv_pooled, vm_early, vm_late, spta_amp, spta_jit, spta_lat, mpta_amp, mpta_jit, mpta_lat, r_vm_velocity, r_hilbert_velocity, r_hilbert_vm
GROUP_A = {
    "label": "Group A",
    "mouse_name": "Jamie11, Vinnie1",
    "frequency_hz": 135,
    "amplitude_uA": 32,
    "pulse_width_us": 60,
    "exposure_ms": "1",
...
SPTA_PEAK_SEARCH_WINDOW_S = (0.0, 0.005)
SPTA_PEAK_TOP_SAMPLES = 2
MPTA_PEAK_SEARCH_WINDOW_S = (0.0, 0.006)
...
def spta_peak_metric_kind(metric_name: str) -> str | None:
    key = re.sub(r"[\s-]+", "_", str(metric_name or "").strip().lower())
    if key in {
        "spta_lat",
        "spta_latency",
        "spta_latency_ms",
        "spta_peak_latency",
        "spta_peak_latency_ms",
        "single_pta_lat",
        "single_pta_latency",
        "single_pta_latency_ms",
    }:
        return "latency"
    if key in {
        "spta_jit",
...
def highest_peak_from_trace(t_rel_s, y, window_s: tuple[float, float]) -> tuple[float, float]:
...
def spta_peak_from_trace(t_rel_s, y) -> tuple[float, float]:
    t = np.asarray(t_rel_s, dtype=float).ravel()
    signal = np.asarray(y, dtype=float).ravel()
    n = min(len(t), len(signal))
    if n == 0:
    ...
    t = t[:n]
    signal = signal[:n]
    start_s, stop_s = SPTA_PEAK_SEARCH_WINDOW_S
    keep = np.flatnonzero((t >= start_s) & (t <= stop_s) & np.isfinite(t) & np.isfinite(signal))
    if len(keep) == 0:
        return np.nan, np.nan
    n_top = max(1, min(int(SPTA_PEAK_TOP_SAMPLES), len(keep)))
    top_idx = keep[np.argsort(signal[keep])[-n_top:]]
    return float(np.nanmean(t[top_idx]) * 1000.0), float(np.nanmax(signal[top_idx]))


def mpta_highest_peak_from_trace(t_rel_s, y) -> tuple[float, float]:
    return highest_peak_from_trace(t_rel_s, y, MPTA_PEAK_SEARCH_WINDOW_S)


def block_spta_peak_arrays(block: BlockSpec) -> tuple[np.ndarray, np.ndarray]:
    data = block.load_summary()
    latencies: list[float] = []
    amplitudes: list[float] = []

    segments = data.get("trials", {}).get("first_pta_segments", [])
    if isinstance(segments, list):
        for segment in segments:
            if not isinstance(segment, dict):
                continue
            latency_ms, amplitude = spta_peak_from_trace(segment.get("t_rel_s"), segment.get("signal"))
...
def block_spta_average_peak(block: BlockSpec) -> tuple[float, float]:
    result = get_single_pta_curve(block)
    if result is None:
        return np.nan, np.nan
    t_rel, y = result
    return spta_peak_from_trace(t_rel, y)


def block_spta_peak_metric_value(block: BlockSpec, metric_name: str) -> float:
    kind = spta_peak_metric_kind(metric_name)
    if kind is None:
        return np.nan
    avg_latency, avg_amplitude = block_spta_average_peak(block)
    if kind == "latency" and np.isfinite(avg_latency):
        return avg_latency
    if kind == "amplitude" and np.isfinite(avg_amplitude):
        return avg_amplitude * float(GEVI_DISPLAY_SCALE)
    latencies, amplitudes = block_spta_peak_arrays(block)
    if kind == "latency":
    ...
    if kind == "amplitude":
        ...
        return float(np.nanmedian(vals) * GEVI_DISPLAY_SCALE) if len(vals) else np.nan
    return np.nan
...
def block_mpta_peak_metric_value(block: BlockSpec, metric_name: str) -> float:
    ...
    if kind is None:
        return np.nan
    if kind == "latency":
        avg_latency, _ = block_mpta_average_peak(block)
        return avg_latency if np.isfinite(avg_latency) else np.nan
    if kind == "jitter":
        latencies, _ = block_mpta_peak_arrays(block)
        vals = latencies[np.isfinite(latencies)]
        return float(np.nanstd(vals, ddof=1)) if len(vals) >= 2 else np.nan
    if kind == "amplitude":
        _, avg_amplitude = block_mpta_average_peak(block)
        return avg_amplitude * float(GEVI_DISPLAY_SCALE) if np.isfinite(avg_amplitude) else np.nan
    return np.nan
...
def block_metric_value(block: BlockSpec, metric_name: str) -> float:
    ...
    if np.isfinite(vm_value):
        return vm_value
    spta_peak_value = block_spta_peak_metric_value(block, metric_name)
    if np.isfinite(spta_peak_value):
        return spta_peak_value
